pageextension 51033 "ACC eInvoices Registers" extends "BLTI eInvoices Registers"
{
    procedure GetSelection(): Text
    var
        eInvoices: Record "BLTI eInvoice Register";
    begin
        CurrPage.SetSelectionFilter(eInvoices);
        exit(GetSelectionFilterForStatisticsGroup(eInvoices));
    end;

    procedure GetSelectionFilterForStatisticsGroup(var eInvoices: Record "BLTI eInvoice Register"): Text
    var
        RecRef: RecordRef;
    begin
        RecRef.GetTable(eInvoices);
        exit(GetSelectionFilter(RecRef, eInvoices.FieldNo("eInvoice No.")));
    end;
