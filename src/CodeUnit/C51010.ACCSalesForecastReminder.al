codeunit 51010 "ACC Sales Forecast Reminder"
{
    ...
    var
        ...
        ExcelFileName: Label 'ACC/%1-%2/%3_%4_Forecast.xlsx';
        ...
        Window: Dialog;
        MsgLabel: Label 'Creating excel file: #1';
        Progress: Label '%1';
    ...
    begin
        ...
        if BusinessUnit.FindSet() then begin
            repeat
                FileName := StrSubstNo(ExcelFileName, BusinessUnit.Code, BusinessUnit.Name, BusinessUnit.Code, Format(TODAY, 0, '<Year4><Month,2><Day,2>'));
                Window.Update(1, StrSubstNo(Progress, FileName));
                // <Header>
                ...
                // SA
                SalesForecast.Reset();
                SalesForecast.SetCurrentKey("Entry No.");
                SalesForecast.SetRange("BLACC Shortcut Dim 2 Code", BusinessUnit.Code);
                ...
                ExcelFooter(ExcelBufferTmp, BusinessUnit.Code, BusinessUnit.Name);
    ...
    begin
        ...
        if BusinessUnit.FindSet() then begin
            repeat
                ...
                ExcelBufferTmp.AddColumn(Format(StartDate, 0, '<Month Text> <Year4>'), false, '', true, false, false, '', ExcelBufferTmp."Cell Type"::Text);
                // </Header>
                ForecastEntry.SetRange(BusinessUnitId, BusinessUnit.Code);
                ForecastEntry.SetRange(ForecastDate, FromDate, ToDate);
                if ForecastEntry.Open() then begin
                    while ForecastEntry.Read() do begin
                ...
                ExcelBufferTmp.WriteSheet(BusinessUnit.Name, 'ACC', UserId);
    ...
    local procedure ExcelFooter(var ExcelBufferTmp: Record "Excel Buffer" temporary; BUCode: Text; BUName: Text)
    var
    begin
        ExcelBufferTmp.CreateNewBook(BUCode);
        ExcelBufferTmp.WriteSheet(BUName, 'ACC', UserId);
