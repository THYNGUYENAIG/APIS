page 51009 "ACC Oversea Import Plan"
{
    ApplicationArea = All;
    Caption = 'Kế hoạch nhập hàng (Tờ khai) - P51009';
    PageType = List;
    ...
    layout
    {
        area(Content)
        {
            repeater(General)
            {
                ...
                field("Posted Purchase Invoice"; PPInvoiceNo) { }
                ...
                field("Full Container Date"; Rec."Full Container Date") { }
                field("Empty Container Date"; Rec."Empty Container Date") { }
                field("Port Date"; Rec."Port Date") { }
                field("Date Adj. Of Port"; Rec."Date Adj. Of Port") { }
                field("Delivery Date"; Rec."Delivery Date") { }
                field("Store lưu bãi"; Rec."Store lưu bãi") { }
                field(DEM; Rec.DEM) { }
                field(DET; Rec.DET) { }
                field(Combine; Rec.Combine) { }
                field("Cont. Type"; Rec."Cont. Type") { }
                field("Product Type"; Rec."Product Type") { }
                field("Cont. 20"; Rec."Cont. 20") { }
                field("Cont. 40"; Rec."Cont. 40") { }
                field("Cont. 45"; Rec."Cont. 45") { }
                ...
                field("C/O Origin Criteria"; Rec."C/O Origin Criteria") { }
                field("Form C/O Type"; Rec."Form C/O Type") { }
                field("Form C/O Num"; Rec."Form C/O Num") { }
                field("Form C/O Date"; Rec."Form C/O Date") { }
                field("C/O Type"; Rec."C/O Type")
                {
                    ToolTip = 'Specifies the value of the C/O Type field.', Comment = '%';
                }
                field("Valid From"; ValidFrom)
                {
                    Caption = 'Ngày chứng thư';
                }
                field("KTCN Reason"; Rec."KTCN Reason") { }
                field("Get Sample"; Rec."Get Sample") { }
                field("Country/Region Code"; Rec."Country/Region Code") { }
                field("P.I.C"; Rec."P.I.C New") { }
                field("P.I.C Name"; Rec."P.I.C Name New") { }
                field("Service Unit"; Rec."Service Unit New") { }
                field("Service Name"; Rec."Service Name New") { }
                field("Purchaser Code"; Rec."Purchaser Code New") { }
                field("Purchaser Name"; Rec."Purchaser Name New") { }
                field("Original Doc. Receipt Date"; Rec."Original Doc. Receipt Date") { }
                field("Cold Running Plug Fee"; Rec."Cold Running Plug Fee") { }
                field("Shipping Line / Agent"; Rec."Shipping Line / Agent") { }
                field("Shipping Line Agent"; Rec."Shipping Line Agent") { }
                field("Local Charge(s)"; Rec."Local Charge(s)") { }
                field(Fee; Rec.Fee) { }
                field("Whse. Reason"; Rec."Whse. Reason") { }
                field("Import Reason"; Rec."Import Reason") { }
                field(Note; Rec.Note) { }
                field("Whse. Note"; Rec."Whse. Note") { }
                field("Storage Fee Reason"; Rec."Storage Fee Reason") { }
                field("Cai Mep Port"; Rec."Cai Mep Port") { }
        ...
        area(processing)
        {
            group(ACCAction)
            {
                ...
                action(ACCCustoms)
                {
                    ...
                    trigger OnAction();
                    var
                        Declaration: Record "BLTEC Customs Declaration";
                        ImportPlan: Record "ACC Import Plan Table";
                    begin
                        ImportPlan.SetFilter("Document No.", Rec."Document No.");
                        if ImportPlan.FindSet() then begin
                            repeat
                                if Declaration.Get(ImportPlan."Document No.") then begin
                                    if Declaration."BLTEC Customs Declaration No." <> ImportPlan."Declaration No." then begin
                                        ImportPlan."Declaration No." := Declaration."BLTEC Customs Declaration No.";
                                        ImportPlan."Declaration Date" := Declaration."Declaration Date";
                                        ImportPlan."Bill No." := Declaration."BL No.";
                                        ImportPlan.Modify();
                                    end;
                                end;
                        ...
                        if ImportPlan.FindSet() then begin
                            repeat
                                if ImportEntry.Get(ImportPlan."Source Document No.", ImportPlan."Source Line No.") then begin
                                    if ImportEntry."BLTEC Customs Declaration No." <> ImportPlan."Declaration No." then begin
                                        ImportPlan."Declaration No." := ImportEntry."BLTEC Customs Declaration No.";
                                        ImportPlan."Declaration Date" := ImportEntry."Customs Declaration Date";
                                        ImportPlan."Bill No." := ImportEntry."BL No.";
                                        ImportPlan.Modify();
                                    end;
                                    if (ImportEntry."Actual ETA Date" <> 0D) AND (ImportPlan."Actual Arrival Date" <> ImportEntry."Actual ETA Date") then begin
                                        if ImportEntry."Process Status" = "BLTEC Import Process Status"::Finished then
                                            exit;
                                        ImportPlan."Actual Arrival Date" := ImportEntry."Actual ETA Date";
    ...
    begin
        ...
        ImportPlan.Reset();
        ImportPlan.SetRange("Declaration Date", FromDate, Today);
        ImportPlan.SetFilter("Declaration No.", '<>%1', '');
        ImportPlan.SetAutoCalcFields("Purchase Item Number", "Purchase Item Name");
        if ImportPlan.FindSet() then
            repeat
                if ImportPlan."Item Number" <> ImportPlan."Purchase Item Number" then begin
                    if ImportPlan."Purchase Item Number" <> '' then begin
                        ImportPlan."Item Number" := ImportPlan."Purchase Item Number";
                        ImportPlan."Item Name" := ImportPlan."Purchase Item Name";
                        ImportPlan.Modify();
                    end;
                end;
            until ImportPlan.Next() = 0;
    ...
    var
        ...
        PurchInvoie: Record "Purch. Inv. Line";
    begin
        PPInvoiceNo := '';
        PurchInvoie.SetRange("Order No.", Rec."Source Document No.");
        PurchInvoie.SetRange("Order Line No.", Rec."Source Line No.");
        PurchInvoie.SetRange("Receipt No.", Rec."Posted Purchase Receipt");
        if PurchInvoie.FindSet() then begin
            repeat
                if PPInvoiceNo = '' then begin
                    PPInvoiceNo := PurchInvoie."Document No.";
                end else begin
                    PPInvoiceNo := PPInvoiceNo + '|' + PurchInvoie."Document No.";
                end;
            until PurchInvoie.Next() = 0;
        end;
        ValidFrom := 0D;
        CDCertificateFile.Reset();
        CDCertificateFile.SetRange("BLACC Customs Declaration No.", Rec."Declaration No.");
        if CDCertificateFile.FindFirst() then begin
            ValidFrom := CDCertificateFile."BLACC Valid From";
        end;
    end;
